...
var replaceEvalString = function(evalString, param) {
  ...
  //---------------------------------------------------------------------------------------
  // (2025-08-09) 
  // evalString 에 있는 변수인데, param 에 넘어오지 않는 변수는 기본적으로 undefined 처리 
  //---------------------------------------------------------------------------------------
  var variables = evalString.match(/[a-zA-Z_][a-zA-Z0-9_]*/g) || [];
  
  for (var i = 0; i < variables.length; i++) {
    var key = variables[i];
    // param 객체에 해당 키가 존재하지 않으면, 임시로 undefined 을 할당하여 ReferenceError를 방지합니다.
    if (!(key in param)) {
      param[key] = undefined;
    }
  }  
